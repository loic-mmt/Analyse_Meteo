...
# ======== Inputs =========
sample_nc = joinpath(@__DIR__, "..", "data", "raw-yearly-combined", "era5_land_hourly", "era5_land_fr_1950_01.nc")
#sample_nc = "../data/raw-yearly-combined/era5_land_hourly/era5_land_fr_1950_01.nc"     # un seul fichier pour la grille
...
# Test d'appartenance d'un point à une géométrie (longitude/latitude -> fichier de formes CRS -> contains)
@inline function inside_geom_ll(geom_prepared, pj_ll_to_src, lon::Float64, lat::Float64)::Bool
    ...
    # Test géométrique: vrai si le point est dans le polygone.
    return geos_contains(geom_prepared, pt)
end

# ======== Calcul par région (1 fichier NetCDF par région) ========

sub_k = 8
step = max(1, Int(floor(0.05 * nlat))) # progrès : toutes les 5% de lignes

for (region_idx, geom_src) in enumerate(geoms)
    println("Region ", region_idx, "/", length(geoms), "...")

    # ======== Conversion en GEOS + prepared geometry ========
    geom_wkt = ArchGDAL.toWKT(geom_src) # WKT car GDAL géométrie != GEOS géométrie
    geom_geos = geos_from_wkt(geom_wkt)
    geom_geos_prepared = geos_prepare(geom_geos) # rapidité
    println("LibGEOS geometry ready (prepared=", !(geom_geos_prepared === geom_geos), ")")
    ...
    @threads for i in 1:nlat
        ...
        for j in 1:nlon
            ...
            latm = 0.5 * (lat0 + lat1)
            center_in[i, j] = inside_geom_ll(geom_geos_prepared, pj_ll_to_src, lonm, latm) # Test au centre.
        end
    ...
    @threads for i in 1:nlat # utilisaion multi-coeur pour maximiser la vitesse d'execution
        for j in 1:nlon
            ...
            for a in 1:sub_k
                x = lon0 + (a - 0.5) * (lon1 - lon0) / sub_k
                for b in 1:sub_k
                    y = lat0 + (b - 0.5) * (lat1 - lat0) / sub_k

                    # `condition ? 1 : 0` = opérateur ternaire: ajoute 1 si dedans, sinon 0.
                    inside += inside_geom_ll(geom_geos_prepared, pj_ll_to_src, x, y) ? 1 : 0
                end
            end

            # Fraction estimée du pixel couverte par la région.
