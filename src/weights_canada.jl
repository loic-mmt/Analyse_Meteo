...
Résumé script
	...
	3.	Lit le shapefile du Canada, fait une union → une géométrie Canada.
	4.	Convertit la géométrie Canada en LibGEOS (plus rapide) et la prépare.
	5.	Calcul en 2 passes :
	    •	Pass 1 : 1 seul test “point dans Canada” au centre de chaque pixel → masque intérieur/extérieur.
...
- teste l'écriture immédiatement (fail-fast) pour éviter d'échouer après un long calcul.
"""
function ensure_output_writable(out_path::AbstractString)
    out_dir = dirname(out_path)
    mkpath(out_dir)
    isdir(out_dir) || error("Dossier de sortie introuvable/non créable: $out_dir")

    try
        mktemp(out_dir) do _, io
            write(io, "ok")
        end
    catch err
        error("Impossible d'écrire dans le dossier '$out_dir': $(sprint(showerror, err))")
    end
end
...
ca_geom_src, pj_ll_to_src = ArchGDAL.read(shp) do dataset
...
ca_wkt = ArchGDAL.toWKT(ca_geom_src) # WKT car GDAL géométrie != GEOS géométrie
ca_geos = geos_from_wkt(ca_wkt)
ca_geos_prepared = geos_prepare(ca_geos) # rapidité
println("LibGEOS geometry ready (prepared=", !(ca_geos_prepared === ca_geos), ")")
# `===` teste l'identité (même objet en mémoire), pas juste l'égalité des valeurs.
# Ici: si `ca_geos_prepared === ca_geos` alors la géométrie n'a pas pu être "préparée" (fallback), sinon elle l'a été.
...
@inline function inside_canada_ll(ca_geos_prepared, pj_ll_to_src, lon::Float64, lat::Float64)::Bool
    ...
    # Test géométrique: vrai si le point est dans le polygone Canada.
    return geos_contains(ca_geos_prepared, pt)
end
...
@threads for i in 1:nlat
    ...
    for j in 1:nlon
        ...
        latm = 0.5 * (lat0 + lat1)
        center_in[i, j] = inside_canada_ll(ca_geos_prepared, pj_ll_to_src, lonm, latm) # Test au centre.
    end
    ...
    for j in 1:nlon
        ...
        for a in 1:k
            ...
            for b in 1:k
                ...
                inside += inside_canada_ll(ca_geos_prepared, pj_ll_to_src, x, y) ? 1 : 0
            end
        end

        # Fraction estimée du pixel couverte par le Canada.
...
NCDataset(out_weights_nc, "c") do wds
    ...
    wds.attrib["description"] = "Canada weights: fraction-in-Canada (subsample) and final_weights=fraction*cos(lat)"
