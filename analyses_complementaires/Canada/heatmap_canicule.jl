...
# Fichiers de poids
const weight_prop_basic = joinpath(PROJECT_ROOT, "data", "masks", "weights_canada_31.nc")
const weight_prop_precise = joinpath(PROJECT_ROOT, "data", "masks", "weights_canada_9.nc")
...
# Shapefile du contour Canada
const CANADA_SHP = joinpath(PROJECT_ROOT, "data", "shapefiles_canada", "lpr_000b21a_f.shp")
...
Charge les segments du contour Canada depuis un shapefile.
...
function calculate_heat_days_streaming(
    ...
    years_set = Set(Int.(collect(year_range)))
    months_set = Set(Int.(collect(selected_months)))

    files = sort(glob("*.nc", data_folder))
    ...
    for (step, file) in enumerate(files)
        NCDataset(file) do ds
            ...
            for _ in 1:n_full_days
                ...
                current_day = Date(times[day_start])
                in_period = (year(current_day) in years_set) && (month(current_day) in months_set)
                ...
                if in_period
                    ...
                    # Une journée hors période casse les séquences en cours.
                    finalize_open_runs!(run_len, canicule_days, min_consecutive_days)
                    fill!(run_len, 0)
                end
...
- masque Canada,
- contour du Canada superposé (si `outline=true`).
"""
function vizumap(
    data_2d,
    weights_file;
...
function save_annual_maps_gif(
    ...
    outline_segments = outline ? load_outline_segments(CANADA_SHP) : Tuple{Vector{Float64}, Vector{Float64}}[]
...
3. trace la heatmap avec contour Canada,
...
function run_canicule_heatmap(;
    ...
    selected_months=collect(1:12),
    output_file::String=joinpath(plot_dir, "heatmap_canicule_canada_1950_2025.png"),
    ...
    p = vizumap(
        ...
        weights_file;
        outline=outline,
        title="Heures de canicule Canada ($(first(year_range))-$(last(year_range)))"
    )
    ...
    if make_gif || make_curve
        if make_gif
            gif_file = isnothing(output_gif_file) ? joinpath(plot_dir, "heatmap_canicule_canada_annual_$(first(years))_$(last(years)).gif") : output_gif_file
            save_annual_maps_gif(
                annual_days_maps,
                years,
                weights_file,
        ...
        if make_curve
            curve_file = isnothing(output_curve_file) ? joinpath(plot_dir, "courbe_canicule_canada_annual_$(first(years))_$(last(years)).png") : output_curve_file
            save_annual_curve(
                years,
                annual_mean_days,
                curve_file;
...
Usage :
- `julia analyses_complementaires/Canada/heatmap_canicule.jl`
- `julia analyses_complementaires/Canada/heatmap_canicule.jl /chemin/sortie.png`
- `julia analyses_complementaires/Canada/heatmap_canicule.jl /chemin/sortie.png 1950 2025`
"""
function main(args=ARGS)
    output_file = length(args) >= 1 ? args[1] : joinpath(plot_dir, "heatmap_canicule_canada_1950_2025.png")
...
julia analyses_complementaires/Canada/heatmap_canicule.jl /tmp/canicule_canada.png

Mode script avec intervalle d'années
julia analyses_complementaires/Canada/heatmap_canicule.jl /tmp/canicule_canada.png 1960 2025

Depuis un autre fichier :
include("analyses_complementaires/Canada/heatmap_canicule.jl")
