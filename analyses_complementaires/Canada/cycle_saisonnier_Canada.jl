...
fichier_poids_canada = joinpath(project_dir, "data", "masks", "weights_canada_31.nc")
...
m_old = compute_general_climatology(data_folder_ca_31, fichier_poids_canada, 1950:1985, mode=:monthly)
v_old = means_vector_calculation(m_old, fichier_poids_canada)
cycle_old = mean(reshape(v_old, 12, :), dims=2)[:, 1]

m_recent = compute_general_climatology(data_folder_ca_31, fichier_poids_canada, 1986:2020, mode=:monthly)
v_recent = means_vector_calculation(m_recent, fichier_poids_canada)
...
plots_saisons = []
for s in saisons
    m = compute_general_climatology(data_folder_ca_31, fichier_poids_canada, years_saison, mode=:yearly, selected_months=s.months)
    v = means_vector_calculation(m, fichier_poids_canada)
